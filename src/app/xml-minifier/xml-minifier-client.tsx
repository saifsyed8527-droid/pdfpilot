...
import { minifyXml } from "@/lib/engines/format-engine";
import type { ResolvedEntity } from "@/lib/content/registry";
import { ToolRelatedContent } from "@/components/content/ToolRelatedContent";

interface XmlMinifierClientProps {
  faqs: FaqInput[];
  related: ResolvedEntity[];
}

export function XmlMinifierClient({ faqs, related }: XmlMinifierClientProps) {
  ...
  const convert = () => {
    ...
    run(
      async () => {
        ...
        setResult(new Blob([minified], { type: "application/xml" }));
      },
      {
        successMessage: "Minified successfully!",
        toolName: "xml-minifier",
        errorTitle: "Failed to minify XML",
  ...
  const downloadResult = () => {
    if (!result) return;
    downloadBlob(result, "minified.xml");
  };
          ...
          <CardContent className="space-y-6">
            ...
            {result && (
              <div className="text-center space-y-4">
                ...
                <h3 className="text-xl font-semibold">Minified successfully!</h3>
