...
import { yamlToJson, jsonToRows, rowsToXml } from "@/lib/engines/data-conversion-engine";
import type { ResolvedEntity } from "@/lib/content/registry";
import { ToolRelatedContent } from "@/components/content/ToolRelatedContent";

interface YamlToXmlClientProps {
  faqs: FaqInput[];
  related: ResolvedEntity[];
}

export function YamlToXmlClient({ faqs, related }: YamlToXmlClientProps) {
  ...
  const convert = () => {
    ...
    run(
      async () => {
        ...
        setResult(new Blob([xml], { type: "application/xml" }));
      },
      {
        successMessage: "Converted to XML successfully!",
        toolName: "yaml-to-xml",
  ...
  const downloadResult = () => {
    if (!result) return;
    downloadBlob(result, "converted.xml");
  };
          ...
          <CardContent className="space-y-6">
            ...
            {file && !result && (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  Your YAML must represent a flat list of objects (a top-level sequence of
                  mappings) — the same tabular scope this project&apos;s other data-format tools share.
                </p>
