...
import { urlDecode } from "@/lib/engines/encoding-engine";
import type { ResolvedEntity } from "@/lib/content/registry";
import { ToolRelatedContent } from "@/components/content/ToolRelatedContent";

interface UrlDecodeClientProps {
  faqs: FaqInput[];
  related: ResolvedEntity[];
}

export function UrlDecodeClient({ faqs, related }: UrlDecodeClientProps) {
  ...
  const convert = () => {
    ...
    run(
      async () => {
        ...
        setResult(new Blob([decoded], { type: "text/plain" }));
      },
      {
        successMessage: "URL decoded successfully!",
        toolName: "url-decode",
        errorTitle: "Failed to URL decode",
  ...
  const downloadResult = () => {
    if (!result) return;
    downloadBlob(result, "url-decoded.txt");
  };
          ...
          <CardContent className="space-y-6">
            ...
            {result && (
              <div className="text-center space-y-4">
                ...
                <h3 className="text-xl font-semibold">URL decoded successfully!</h3>
