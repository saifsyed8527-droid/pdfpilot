...
import { flattenPdfForm } from "@/lib/engines/pdf-engine";
import type { ResolvedEntity } from "@/lib/content/registry";
import { ToolRelatedContent } from "@/components/content/ToolRelatedContent";

interface FlattenPdfClientProps {
  faqs: FaqInput[];
...
export function FlattenPdfClient({ faqs, related }: FlattenPdfClientProps) {
  ...
  const flattenPdf = () => {
    ...
    run(
      ...
      },
      {
        ...
        errorTitle: "Failed to flatten PDF",
        onError: (error) =>
  ...
  const downloadResult = () => {
    if (!resultPdf) return;
    downloadBlob(resultPdf, "flattened.pdf");
  };
          ...
          <CardContent className="space-y-6">
            ...
            {file && !resultPdf && (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  This locks in the current values of every form field so they can no longer be
                  edited — the fields become regular page content.
                </p>

                {processing && (
                  <Progress value={progress} className="h-2" aria-label="Flattening PDF" />
                )}
            ...
            {resultPdf && (
              <div className="text-center space-y-4">
                ...
                <h3 className="text-xl font-semibold">PDF flattened successfully!</h3>
