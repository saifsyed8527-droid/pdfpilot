...
import { formatJson } from "@/lib/engines/format-engine";
...
export function JsonFormatterClient({ faqs, related }: JsonFormatterClientProps) {
  ...
  const convert = () => {
    ...
    run(
      async () => {
        ...
        setResult(new Blob([formatted], { type: "application/json" }));
      },
      {
        successMessage: "Formatted successfully!",
        toolName: "json-formatter",
        errorTitle: "Failed to format JSON",
  ...
  const downloadResult = () => {
    if (!result) return;
    downloadBlob(result, "formatted.json");
  };
          ...
          <CardContent className="space-y-6">
            ...
            {file && !result && (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  Re-indents JSON with 2-space formatting for readability. Malformed JSON produces
                  a clear parse error instead of a broken result.
                </p>
