...
import { minifyJson } from "@/lib/engines/format-engine";
import type { ResolvedEntity } from "@/lib/content/registry";
import { ToolRelatedContent } from "@/components/content/ToolRelatedContent";

interface JsonMinifierClientProps {
  faqs: FaqInput[];
  related: ResolvedEntity[];
}

export function JsonMinifierClient({ faqs, related }: JsonMinifierClientProps) {
  ...
  const convert = () => {
    ...
    run(
      async () => {
        ...
        setResult(new Blob([minified], { type: "application/json" }));
      },
      {
        successMessage: "Minified successfully!",
        toolName: "json-minifier",
        errorTitle: "Failed to minify JSON",
  ...
  const downloadResult = () => {
    if (!result) return;
    downloadBlob(result, "minified.json");
  };
