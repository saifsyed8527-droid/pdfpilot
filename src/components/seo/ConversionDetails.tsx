import { ToolGrowthLinks } from "./ToolGrowthLinks";
...
import { templatesForTool, TEMPLATE_ROOT } from "@/lib/content/conversion-templates";
...
export function ConversionDetails({ tool, locale = "en" }: { tool: ConversionToolSlug; locale?: LocaleCode }) {
  ...
  return <><section id="conversion-details" data-conversion-details={tool} lang={locale} dir={getLocale(locale)?.dir} className="border-t bg-background px-4 py-12 text-foreground">
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      <nav aria-label="Conversion templates" lang="en" dir="ltr"><h2 className="text-xl font-semibold"><Link href={`${TEMPLATE_ROOT}/${tool}`}>Ready-to-use conversion templates (English)</Link></h2><ul className="mt-4 grid gap-3 sm:grid-cols-2">{templatesForTool(tool).map(page => <li key={page.slug}><Link href={page.path} hrefLang="en" className="underline underline-offset-4 hover:text-primary">{page.title}</Link></li>)}</ul></nav>
    </div>
  </section><ToolGrowthLinks tool={tool} /></>;
}
